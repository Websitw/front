...
import '../../Admin/Dashboard/Navbar/DashboardNavbar.css';
    ...
    <nav className="dashboard-navbar">
      <div className="navbar-left">
        ...
        <div className="dashboard-info">
          ...
          <p className="welcome-text">{t('dashboard.welcomeBack')} {user ? user.name : ''}</p>
          <div className="breadcrumb">
            <Home size={14} />
            <span>{t('dashboard.home')}</span>
            <ChevronRight size={14} />
            <span className="breadcrumb-current">{t('dashboard.dashboard')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="navbar-right">
        <button className="icon-button" onClick={toggleLanguage}>
          <span className="lang-toggle">{i18n.language === 'en' ? 'AR' : 'EN'}</span>
        </button>

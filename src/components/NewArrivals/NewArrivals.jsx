...
const NewArrivals = () => {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.get(
        `${environment.serverOrigin}details/shopProductsTab?from=2025-05-20&to=2025-12-20`,
        {
          headers: {
      ...
      console.log("error when get products NewArrivals  ", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getOrCreateCart = async (token, userId) => {
    ...
    if (
      !pendingCartResponse?.data?.items ||
      pendingCartResponse?.data?.items?.length === 0
    ) {
  ...
  const addOrUpdateCartItem = async (token, cartId, cartItem) => {
    ...
    if (existingProduct) {
      ...
      await axios.post(`${baseUrl}shopCartItemsTab`, DTO, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        <div className="new-arrivals">
          <div className="header">
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <span>{t("newArrivals.subtitle")}</span>
            </div>
            <span
              ...
              {data.slice(0, 6).map((p) => (
                <div className="card" key={p.id}>
                  ...
                  <img
                    ...
                    src={
                      p?.iconPath
                        ? `${baseUrl}_xfilestore/mada/${p.iconPath}`
                        : ""
                    }
                    alt={p.title}
                  />
                  ...
                  <div className="price-box">
                    ...
                    {p.discount > 0 && (
                      <span className="old-price">JOD {p.price}</span>
                    )}
                  </div>
                  <button
                    onClick={() => {
